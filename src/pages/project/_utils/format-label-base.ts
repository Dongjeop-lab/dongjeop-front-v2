...
export const formatChairTypes = (labelInfo: LabelBase | null): string => {
  ...
  const types = [];
  if (labelInfo.has_movable_chair) types.push('낮은 이동식 의자');
  if (labelInfo.has_high_chair) types.push('높은 이동식 의자');
  if (labelInfo.has_fixed_chair) types.push('고정식 의자');
  if (labelInfo.has_floor_chair) types.push('좌식 의자');
